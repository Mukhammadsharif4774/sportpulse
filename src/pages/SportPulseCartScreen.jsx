...
import SportPulseCartItemComponent from '../components/SportPulseCartItemComponent';
...
export default function () {
  ...
  const handleOrder = () => {
    const destinationScreen = cart.length
      ? 'SportPulseCartSuccessScreen'
      : 'SportPulseHomeScreen';
        ...
        <>
          <View style={{height: height * 0.7}}>
            <ScrollView style={styles.flex} contentContainerStyle={styles.main}>
              {cart.map((item, index) => (
                <SportPulseCartItemComponent item={item} key={index} />
              ))}
            </ScrollView>
          </View>
        </>
